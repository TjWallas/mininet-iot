...
class propagationModel(object):
    ...
    def __init__(self, node1=None, node2=None, dist=0, wif=0):
    ...
    def pathLoss(self, node1, dist, wif):
        """Path Loss Model:
        (f) signal frequency transmited(Hz)
        (d) is the distance between the transmitter and the receiver (m)
        (c) speed of light in vacuum (m)
        (L) System loss"""
        f = node1.params['freq'][wif] * 10 ** 9  # Convert Ghz to Hz
        c = 299792458.0
        L = self.sL
    ...
    def friis(self, **kwargs):
        ...
        (c) speed of light in vacuum (m)
        (L) System loss"""
        gr = kwargs['node1'].params['antennaGain'][kwargs['wif']]
        pt = kwargs['node2'].params['txpower'][0]
        gt = kwargs['node2'].params['antennaGain'][0]
        d = kwargs['dist']
        gains = pt + gt + gr

        pl = self.pathLoss(kwargs['node1'], d, kwargs['wif'])
        self.rssi = gains - pl
    ...
    def twoRayGround(self, **kwargs):
        ...
        a short distance)"""
        gr = kwargs['node1'].params['antennaGain'][kwargs['wif']]
        hr = kwargs['node1'].params['antennaHeight'][kwargs['wif']]
        pt = kwargs['node2'].params['txpower'][0]
        gt = kwargs['node2'].params['antennaGain'][0]
    ...
    def logDistance(self, **kwargs):
        ...
        pl = self.pathLoss(kwargs['node1'], ref_d, kwargs['wif'])
    ...
    def ITU(self, **kwargs):
        ...
        pt = kwargs['node2'].params['txpower'][0]
        gt = kwargs['node2'].params['antennaGain'][0]
        f = kwargs['node1'].params['freq'][kwargs['wif']] * 10 ** 3
...
class GetSignalRange(object):
    ...
    def __init__(self, node=None, wif=0, enable_interference=False):
        "Calculate the signal range given the propagation model"
        if ppm.model in dir(self):
            self.__getattribute__(ppm.model)(node=node, wif=wif,
    ...
    def friis(self, **kwargs):
        ...
        (L) System loss"""
        # Convert Ghz to Hz
        f = kwargs['node'].params['freq'][kwargs['wif']] * 10 ** 9
        txpower = kwargs['node'].params['txpower'][kwargs['wif']]
        gain  = kwargs['node'].params['antennaGain'][kwargs['wif']]
    ...
    def pathLoss(self, node, dist, wif):
        """Path Loss Model:
        (f) signal frequency transmited(Hz)
        (d) is the distance between the transmitter and the receiver (m)
        (c) speed of light in vacuum (m)
        (L) System loss"""
        f = node.params['freq'][wif] * 10 ** 9  # Convert Ghz to Hz
        c = 299792458.0
        L = ppm.sL
        lambda_ = c / f  # lambda: wavelength (m)
        denominator = lambda_ ** 2
    ...
    def twoRayGround(self, **kwargs):
        """Two Ray Ground Propagation Loss Model (does not give a good result for
        a short distance)"""
        gt = kwargs['node'].params['antennaGain'][kwargs['wif']]
        ht = kwargs['node'].params['antennaHeight'][kwargs['wif']]
        pt = kwargs['node'].params['txpower'][kwargs['wif']]

        if 'rssi' in kwargs['node'].params:
            rssi = kwargs['node'].params['rssi'][kwargs['wif']]
        else:
            rssi = -60
    ...
    def logDistance(self, **kwargs):
        ...
        pl = self.pathLoss(kwargs['node'], ref_d, kwargs['wif'])
    ...
    def logNormalShadowing(self, **kwargs):
        ...
        if kwargs['interference']:
            ...
            w_server.update_gaussian_random(
                WmediumdGRandom(kwargs['node'].wmIface[kwargs['wif']],
                                gRandom))

        pl = self.pathLoss(kwargs['node'], ref_d, kwargs['wif']) - gRandom
    ...
    def ITU(self, **kwargs):
        """International Telecommunication Union (ITU) Propagation Loss Model:"""
        f = kwargs['node'].params['freq'][kwargs['wif']] * 10 ** 3
        txpower = kwargs['node'].params['txpower'][kwargs['wif']]
        gain  = kwargs['node'].params['antennaGain'][kwargs['wif']]
        gains = txpower + (gain * 2)
        N = 28  # Power Loss Coefficient
...
class GetPowerGivenRange(object):
    ...
    def __init__(self, node, wif, dist, enable_interference):
        "Calculate txpower given the signal range"
        if ppm.model in dir(self):
            self.__getattribute__(ppm.model)(node=node, wif=wif, dist=dist,
                                             interference=enable_interference)
    ...
    def logNormalShadowing(self, **kwargs):
        ...
        if kwargs['interference']:
            ...
            w_server.update_gaussian_random(WmediumdGRandom(
                kwargs['node'].wmIface[kwargs['wif']], gRandom))
