...
function wifi_deps {
    ...
    pushd $MININET_DIR/mininet-iot/hostap/hostapd
    cp defconfig .config
    sudo make && make install
    pushd $MININET_DIR/mininet-iot/hostap/wpa_supplicant
    cp defconfig .config
    sudo make && make install
    pushd $MININET_DIR/mininet-iot/
    git clone --depth=1 https://github.com/ramonfontes/iw
    pushd $MININET_DIR/mininet-iot/iw
    sudo make && make install
    cd $BUILD_DIR
...
function of {
    ...
    patch -p1 < $MININET_DIR/mininet-iot/util/openflow-patches/controller.patch
...
function install_wireshark {
    ...
    cp -n $MININET_DIR/mininet-iot/util/colorfilters $HOME/.wireshark
...
function nox {
    ...
    git am $MININET_DIR/mininet-iot/util/nox-patches/*tutorial-port-nox-destiny*.patch
    if [ "$DIST" = "Ubuntu" ] && version_ge $RELEASE 12.04; then
        git am $MININET_DIR/mininet-iot/util/nox-patches/*nox-ubuntu12-hacks.patch
    fi
...
function wmediumd {
    ...
    git clone --depth=1 -b mininet-iot https://github.com/ramonfontes/wmediumd.git
...
function usage {
    ...
    printf -- ' -W: install Mininet-IoT dependencies\n' >&2
